...
- **Writing applications** — `docs/lpc-essentials.md` (LPC language orientation, bridges to the formal spec), `docs/kernel-libraries.md` (the inheritable libraries under `src/lib/`), `docs/application-authoring.md` (general tier-E application patterns, non-HTTP transports), `docs/application-repository.md` (your application's own repository beside the platform tree: deploy, CI, and update flows), `docs/http-applications.md` (HTTP/1-specific patterns)
- **Operations** — `docs/operations.md` (boot modes, state persistence, logging, extension loading), `docs/configuration.md` (`.dgd` configuration fields, resource limits, capacity ceilings), `docs/admin-console.md` (operator console: connecting, console security posture, per-task operational reference, verb appendix)
...
- **Stock-build ceilings.** 255 simultaneous users, 65535 objects, and a swap device capped at 65535 sectors -- about 64 MiB of pageable object storage at the demo config's 1 KiB sector size, scaling only through `sector_size`, whose own compiled cap of 65535 bytes puts total persistent state on a stock build just under 4 GiB (`docs/configuration.md` Limits and capacity states each ceiling and which rows have headroom).
