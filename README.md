...
- **Stock-build ceilings.** 255 simultaneous users, 65535 objects, and a swap device capped at 65535 sectors -- about 64 MiB of pageable object storage at the demo config's 1 KiB sector size, scaling only through `sector_size`, whose own compiled cap of 65535 bytes puts total persistent state on a stock build just under 4 GiB ([`docs/configuration.md`](docs/configuration.md#limits-and-capacity) Limits and capacity states each ceiling and which rows have headroom).
