...
11. `LPC_EXT_CRYPTO=<crypto-module> EXPECTED_OK=51 DGD_BIN=<dgd> scripts/run-example.sh composite-app` -- `PASS`, 51 sentinels: the full composite set (WebAuthn/session authentication over the wire, the agent self-service panel incl. the standing refusal, passkey self-service incl. enrollment, the capability-gated report refusal, the server-sent-event streams, the identity mutation-event phases, the recovery ceremony, and the restore boot). Without the module the subset at step 10 is the documented fallback.
