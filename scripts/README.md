...
```sh
scripts/drive-verbs-smoke.sh                  # default: all seven verbsets over vault-app + upgrade-cascade deploys
...
8. `DGD_BIN=<dgd> scripts/drive-verbs-smoke.sh` -- `DRIVE-VERBS PASS` after the seven default verbsets (admin-baseline, logging-verbs, schema-verbs, dispatcher-verbs, port-labels, operator-provision, operator-upgrade) run against the vault-app (MyApp) and upgrade-cascade (Cascade) deploys.
