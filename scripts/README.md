...
The doc-drift guard for the two-tutorial application-authoring path: `docs/first-application.md` and `docs/first-http-endpoint.md`. `scripts/tutorial-replay.py`, the python driver this script invokes, parses the command/expected-output pairs out of both docs' fenced transcripts AT RUN TIME -- no generated mirror of the transcripts is kept anywhere, because a mirror would drift out of sync with the docs, which is exactly the failure this guard exists to catch. Every fence in both docs is classified: `text`/`sh`/`c` is the complete whitelist both tutorials use (derived by inspecting every ` ``` ` marker in both files), and a fence tagged with anything else is an immediate named FAIL (file, line, language), before any boot -- the defect this guard exists for is a fence-kind rename silently dropping a whole class of assertions while some other fence kind in the same document keeps the block count nonzero, so the whitelist check runs first, ahead of any per-document zero check. A ```` ```text ```` fence is a console transcript (each `# <cmd>` line plus the output lines that follow it, up to the next `# ` line); a ```` ```sh ```` fence is either a plain setup command (`mkdir`, `cp`), a `curl` command paired with its `# <expected>` comment line, or the tutorial's `dgd example.dgd state/snapshot...` restart line (parsed for its snapshot-file arguments); a ```` ```c ```` fence with no `<path>.c` target named in its anchor sentence (the preceding non-blank line) is illustrative quoting of already-shipped code and reports a named SKIP with its reason, while a ```` ```c ```` fence that does name a target is landed by one of four phrasings the anchor sentence must affirmatively match -- "replace the whole `NAME()` function", "above `NAME`" (insert before a declaration), "after the `NAME` branch" (insert after an `if (method == "NAME")` block), or an anchor opening with the backtick path itself followed by a comma-led description (the whole-file-write case, initd.c/kv_daemon.c) -- and a pathed fence matching none of the four is a named FAIL quoting the anchor, never a guessed whole-file overwrite. The two tutorials run back to back in one clean-slate boot, replaying both of their internal `reboot` + snapshot-restore cycles as real process restarts and driving the console half over telnet (importing `drive-verbs.py`'s `Session`/`login` machinery directly, so one persistent session carries the `$N` console-history state a fresh connection resets -- see `drive-verbs.py`'s own module docstring for the login flow this reuses) and the HTTP half via `curl` subprocesses.

Port and value normalization live entirely in `tutorial-replay.py`: any parsed command or expected line embedding the tutorials' literal teaching ports (8023, 8080) is rewritten to `SMOKE_TELNET_PORT`/`SMOKE_BINARY_PORT` before use, and the one run-varying expectation class the two docs contain -- DGD console history slots (`$N = ...`, which the tutorials themselves warn shift with every extra console command and reset to `$0` on each fresh connection) -- is normalized by comparing only the value to the right of `=`, never the slot number. What is loud, precisely: an unrecognized fence language is a named FAIL (above); a recognized-but-unassertable block is a named SKIP with its reason; a pathed c-fence with an unmatched anchor is a named FAIL quoting the anchor; a comparator mismatch is a named FAIL naming the block. A parse yielding zero command/expected-output blocks for either document is itself a FAIL too (the empty-parse guard, belt-and-braces alongside the whitelist check). `TUTORIAL-SMOKE PASS (first-application: N console + M http, first-http-endpoint: P console + Q http)` is the pass signal; boot output lands in `state/tutorial-smoke-boot.log`.
...
31. `DGD_BIN=<dgd> scripts/tutorial-smoke.sh` -- `TUTORIAL-SMOKE PASS (first-application: N console + M http, first-http-endpoint: P console + Q http)` after replaying every documented command/expected-output pair in `docs/first-application.md` and `docs/first-http-endpoint.md`, parsed from the docs' own fenced transcripts at run time, including both tutorials' `reboot` + snapshot-restore cycles as real process restarts. See `tutorial-smoke.sh` below for the parse strategy and normalization rules.
