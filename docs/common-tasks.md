...
3. Rotation is a file replacement (plus a re-read if held). A cold boot needs no step: the file survives it -- the property a console-set, image-only secret lacks: a cold boot rebuilds from the tree, and image-only state does not come back (`docs/changing-a-running-system.md`, the image-versus-source drift discussion).
