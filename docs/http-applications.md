...
**Streaming teardown: the doneChunk drain contract.** `sendChunk`/`endChunk` defer through a zero-delay `call_out`, and the write they trigger completes as a later network event -- the send is never done by the time the call that issued it returns. Ending a pushed stream is therefore two steps, not one: send the terminating (length-0) chunk, then wait for the relay callback `void doneChunk()` before tearing the relay object down. `Connection1`'s `messageChunk()` calls `relay->doneChunk()` every time `outbuf` fully drains, not only on the final drain -- a stream that pushed several frames sees several `doneChunk` calls before the terminating chunk's own drain. A relay that destructs on the first (or any) `doneChunk` call, or that destructs itself in the same task as `endChunk()` without waiting for `doneChunk` at all, drops data: bytes still in `outbuf` die with the object, the peer is left half-open with no EOF, and pushes wrapped in `catch` surface nothing. The relay gates teardown with its own closing flag -- set the flag when sending the terminating chunk, destruct only when `doneChunk` fires with the flag set -- so a mid-stream drain is a no-op and the terminal drain is the one that tears down. This holds even when the terminating chunk is the only thing ever written (an already-idle stream): `endChunk` is itself deferred, so the drain signal still arrives asynchronously after that chunk flushes, never inline with the call that sent it. The composite example's `end_stream()` / `doneChunk()` pair (`examples/composite-app/WWW/obj/server.c`, duplicated in `obj/tls_server.c`) is the worked form; `docs/composite-applications.md` The event streams covers the broker side. The closing flag itself is armed synchronously inside `end_stream()`, but the terminal chunk it guards is only queued at that point, through the same zero-delay `call_out` as any other write, and same-delay `call_out` order is explicitly unguaranteed (`docs/execution-model.md` Which ready task runs next, "Same-delay order is explicitly unguaranteed"). One narrow interleaving therefore remains open: a write already in flight when the flag is set can deliver its own drain signal between the flag going up and the terminal chunk reaching the wire, and that drain reads as the terminal one, tearing the relay down a beat early. Telling that drain apart from the real terminal one would need the connection library to carry a final-drain marker of its own; it does not carry one today.
