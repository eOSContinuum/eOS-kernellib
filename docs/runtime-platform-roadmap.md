...
- **Observer-storage formalization.** The observer read side and lifecycle contract are formalized. Three public read-only query LFUNs on the Merry daemon (`query_observers`, `query_effective_observers`, `query_observed_paths`) expose the local slot, the ancestry-walk view, and observed-path enumeration, all with descriptive returns; `remove_observer` adds by-index removal beside the coarse clear; `register_observer` accepts an array of paths as cross-property registration sugar (compile once, per-slot storage unchanged); the `observers` verb gains observed-path enumeration and an `-effective` walk view, and `unregister-observer` an optional index. Multi-inheritance is stated as an explicit single-parent contract, to revisit only if the hierarchy grows multiple parents. See `docs/observers.md` for the lifecycle contract (storage, lookup, mutation, persistence, eviction).
