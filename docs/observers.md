...
**Limitation -- per-node eviction stays public.** The bulk pass, `clean_nodes`, is `static`: only the daemon's own pool-bound callout runs it, so cross-domain code cannot force recompile churn on the whole pool. The per-node `suicide()` on the compiled program itself remains public (callable by anything holding a program reference, with no pool-bound trigger involved) -- the merry-app eviction phase drives it directly, and holding a program reference is already a narrow position. Either way the exposure is transient cost (a forced recompile at next fire), not correctness.
