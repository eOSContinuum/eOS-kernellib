...
| New connections open but nothing answers; existing service goes quiet | The `users` table is full: genuine concurrent load at the cap, or a leak outpacing reclaim -- a server that never releases completed requests holds each slot until the flow layer's 60-second inactivity timeout | The `status` verb's Users line (or the health route's `users=` line) pinned at the cap. Find the leak (each HTTP example releases via `doneRequest()` when its response completes); a reboot from snapshot clears the table but not the cause |
