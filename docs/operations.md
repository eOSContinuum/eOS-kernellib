...
2. **Write the production configuration.** Start from the production-shape starting point (`docs/configuration.md` Limits and capacity): size the caps, decide the deployment's snapshot cadence against the availability model's recovery-point and recurring-pause trade (Availability and data-loss model below; the platform schedules no snapshots itself, so the cadence is something you run -- an application trigger_dump loop or host-side automation), keep `telnet_port` bound to loopback, and name the `modules`.
...
| `objects` lower | **Refused** when the new cap is below the object count the snapshot holds: `Too many objects in restore file (<count>)`. With the raise above, this makes `objects` an upward-only ratchet across a restore -- coming back down needs a cold boot |
...
**Why `typechecking` is the one that refuses.** A restore compares the first four bytes of the snapshot header: a validity marker, the format version -- copied across from the read header before the comparison, so it never differs -- the driver model, and `typechecking` (`src/config.h`'s snapshot header, compared in `src/config.cpp`). No other `.dgd` field is in those bytes, which is why the rest of the table reads as "accepted". The operator-facing rule is three lines: freeze `typechecking` for the life of a deployment, treat `objects` as a cap that only moves up while restoring, and remember that the driver binary is part of the contract as much as the config file is.

**The driver build travels with the snapshot.** A snapshot written by a stock driver restores under one rebuilt with wider index types, so widening is a forward migration that keeps live state. The reverse does not hold: a wide-written snapshot offered back to a stock driver refuses with `Config error: initialization failed` -- cleanly, with no crash and no partial restore, which is the behavior to rely on. Nothing warns before the attempt, so keep a pre-migration snapshot and treat a wider rebuild as a one-way deploy (`docs/building.md` Wider index types).
...
- `shutdown(int hotboot)`: shut down the platform. The kfun does not snapshot before exiting. Whether the next boot has a snapshot to restore from depends on whether one was taken (by the `snapshot` or `reboot` verbs, by an application `trigger_dump` call, or by the SIGTERM interrupt dump). With non-zero argument and a `hotboot` tuple in the `.dgd` config, performs a hot boot via `execv` instead of exiting. If the `hotboot` tuple is absent, the kfun raises "Hotbooting is disabled".
...
The driver schedules no snapshots of its own -- `dump_interval` governs post-restore rebuild spreading, not a dump timer (`docs/persistence.md` The rebuild interval) -- so every snapshot on disk traces to a verb, an application call, or a SIGTERM stop. The previous snapshot is moved to `<dump_file>.old` before the new one is written. If the running snapshot is corrupt, deleting it leaves `.old` in place for restore on next boot.
...
|---|---|---|
| Kill signal (SIGTERM) | `prepare_reboot()` then `dump_state(1)` (`src/kernel/sys/driver.c:762-771`) | `dump_file` + `dump_file.old` (in practice, see below) |
...
1. **Trigger a deliberate full dump headlessly.** `scripts/drive-verbs.py` (the regression harness's telnet client, `scripts/README.md`) logs into the console and drives verbs from a file: a one-line ephemeral verbset -- `cmd: snapshot`; the verb succeeds silently, and step 2's mtime move is its acknowledgment -- makes the backup generation self-sufficient (the `snapshot` verb writes a full snapshot, the stop-path table above), rather than assuming a recent snapshot exists (the platform schedules none itself). The trigger authenticates as an operator (file-level `user:`/`password:` directives); remember the monitoring-credential caveat under Monitoring signals -- a console credential is never read-only in blast radius.
...
4. **Close the race.** A concurrently triggered dump -- an application cadence loop, another operator -- can rotate the pair mid-copy. After the copy, re-read `dump_file`'s mtime; if it moved during the copy, redo the copy. With a cadence sized in hours the retry is rare.
...
**Recovery point.** The RPO is the age of the last completed dump. The platform supplies no snapshot schedule (`docs/persistence.md` The statedump cycle), so the RPO is a cadence the deployment runs -- an application `trigger_dump` loop or host-side automation driving the `snapshot` verb -- and sizing that cadence is the operator's decision, not a platform-supplied guarantee. Work committed after the last completed dump is lost on an unclean stop; a clean SIGTERM stop writes a final dump on the way down, but a crash, SIGKILL, or power loss does not, and a deployment running no cadence has an unbounded loss window reaching back to its last deliberate dump. The prior snapshot is untouched by a failed or interrupted dump attempt and remains a valid restore point (Backing up and restoring state above).
...
**Availability arithmetic, worked.** The pieces above, composed into three numbers for one concrete deployment shape: a 506 MB image, stock `sector_size`, and a one-hour snapshot cadence (however the deployment runs it: an application `trigger_dump` loop or scheduled operator dumps). The numbers are this rig's, measured on the hardware named in `docs/configuration.md` Snapshot-pause scaling -- not a guarantee for any other machine or workload.

- **Planned pause time per day.** A one-hour cadence runs 24 dumps a day; the stock-`sector_size` client-observed pause at this image size is the top of the measured 0.003-0.037 s range (`docs/configuration.md` Snapshot-pause scaling, measured across two sector sizes). 24 x 0.037 s is about 0.9 s of runtime-blocked time per day.
- **Worst-case unclean-stop loss.** The RPO is the cadence itself ("The RPO is the age of the last completed dump", Recovery point above): up to one hour of committed-but-undumped writes, including writes already acknowledged to clients -- and only while the cadence keeps running; a silently dead cadence loop grows this window without bound (the stalled-snapshot monitor below exists for exactly that).
...
`SIGINT`, `SIGHUP`, `SIGUSR1`, and `SIGUSR2` are not caught: their default disposition applies, so a stop sent as `SIGINT` terminates the process without the snapshot. Only `SIGTERM` runs the snapshot-and-shutdown path. `SIGINT`, `SIGKILL`, and a host crash bypass it and lose the work committed since the last completed dump (Availability and data-loss model above).
...
| `dump_file` modification time | Stalled | Page when the mtime is older than the deployment's snapshot-cadence interval plus the measured dump duration | The snapshot cadence has stopped completing -- the loop or automation driving it died, or the write fails (disk-full, permissions) (A stalled snapshot below) |
...
**A stalled snapshot.** The most reliable unattended signal that the deployment's snapshot cadence has stopped is the `dump_file` modification time. A successful dump rewrites it (every cadence dump, plus any explicit one), and the rotation moves the prior file to `<dump_file>.old` first (State persistence above). A `dump_file` mtime older than the cadence interval plus the dump duration means dumps are no longer completing -- either the source of the cadence died (an application `trigger_dump` loop that stopped re-arming, host automation that quit) or the write itself fails, most often disk-full or a permissions problem on the dump directory (Common failure modes below). Because the rotation writes `<dump_file>.old` before the new file, a failed write leaves the prior snapshot intact as a restore point. An operator-invoked `snapshot` that fails also reports to the console and, through `errord`, to `system.log`.
...
3. **Rebuild the driver with wider index types.** This is the move that raises the compiled sector and object ceilings, and it has a working recipe validated on both supported platforms (`docs/building.md` Wider index types). Price it as a deploy rather than a config change: the migration is forward-only, since a wide-written snapshot will not restore under a stock driver (Config changes across a restore, above); it costs a few percent of snapshot size plus a per-object memory increment that scales with the graph; and it does not move the 255-connection ceiling, which is a different compile-time type. Rehearse the restore off-host first, as with any binary swap (Replace the host binary, end to end, above).
...
- a **streaming subscriber** holds a slot for the lifetime of the stream, by design (`docs/composite-applications.md` The event streams): a dashboard tab with an open SSE subscription is one slot for as long as the tab lives. The inactivity timeout does not bound that lifetime -- the streaming entry point suspends it for the stream's life and `doneRequest()` resumes it -- so a stream's slot comes back when the peer closes and that close is seen on the re-armed read, not on a 60-second window. A peer that vanishes without closing at all is caught by neither path and holds its slot until the application probes for it (`docs/http-applications.md` Release each completed request with `doneRequest()`);
...
The ceiling itself is compiled into the driver: the config parser refuses `users` past 255 at any boot (Booting above; Common failure modes carries the at-the-cap row), because the field's permitted range is `EINDEX_MAX`, a one-byte count. The wider-index rebuild at `docs/building.md` Wider index types does not help here -- it widens the object and sector indices, leaving `EINDEX_TYPE` alone -- so reaching for it in response to a connection-slot alarm raises ceilings that were not binding. Which they are not, by a wide margin: measured against this project's composite example driven through an authenticated write path, connection slots exhaust roughly two orders of magnitude before the object table does, which sits at about 1.3% of its compiled ceiling at 250 concurrent connections. Plan the fronting shape and the workload's slot profile, not a bigger ceiling.
