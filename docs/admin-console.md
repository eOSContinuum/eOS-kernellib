...
- **Observer audit**: `observers /usr/MerryApp/obj/parent foo` shows every compiled observer registered for the `foo` property on `parent`, grouped by timing. Useful before a Merry-script redeploy to confirm what the substrate believes about the current registration state.
