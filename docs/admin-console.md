...
- `status <obj>` calls `status(obj)` for per-object status: program path, owner, clone count, dataspace size. `<obj>` is an LPC path, a `$N`/`$ident` history reference, or an Index logical name (`status Schema:Core:Entry`); colon-shaped arguments resolve path-first, then through the logical-name registry, and print `No such object or Index name.` when neither route resolves.
