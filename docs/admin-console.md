...
The kernel admin console itself remains MERRY-unaware. Other operator surfaces extend the registry's hardcoded verb table the same way: logd hosts `log` and `log-level`, the HTTPS bootstrap hosts `tls-cert` (certificate and registration status; `tls-cert reload` revalidates the PEM files and completes a registration the boot deferred for want of a certificate, without a restart; `tls-cert paths <cert> <key>` re-points the configured paths and reloads), identityd hosts `identity` (registry status; `identity mint <n>` creates an identity record whose initial credentials are `n` recovery codes, printing the plaintext codes once; `show`, `rotate-codes`, `redeem`, and `revoke` cover the record lifecycle; `bind <uuid> <challenge> <cdj-b64u> <ao-b64u>` is the operator-mediated re-bind, verifying the new passkey's registration ceremony through webauthnd before binding it to the existing record; `grant <uuid> <capability>` and `ungrant` bind a platform capability to the identity's principal through the KERNEL()-gated store -- `docs/system-daemons.md` identityd, `docs/capability.md` Identity principals), webauthnd hosts `webauthn` (relying-party status and configuration via `rpid`/`origin`; `register` and `authenticate` drive the ceremonies from base64url payloads, the operator-mediated ceremony path -- `docs/system-daemons.md` webauthnd), sessiond hosts `session` (session-count and mint-ceiling status; `mint <principal> [ttl]` prints a bearer token once, `max-ttl [seconds]` shows or sets the mint ttl ceiling -- runtime state that persists across statedumps, resets to the one-day default at cold boot, and clamps only future mints -- and `validate`, `revoke`, `revoke-principal`, and `list <principal>` -- per-session bookkeeping rows of id, created, expires, never a token -- cover the rest -- `docs/system-daemons.md` sessiond), and capabilityd hosts `capability` (the capability-store listing, each capability with its principal count and delegable flag; `capability delegable <cap> on|off` flips the per-capability delegation gate consumed by controller-to-agent delegation; `capability grant <cap> <principal>` and `capability revoke` are the operator path for domain and program principals, the ones `identity grant`'s identity-constrained seam refuses -- `docs/system-daemons.md` capabilityd, `docs/capability.md` Identity principals and the operator grant path). Future Vault or Schema surfaces follow the same shape and seed their own capabilities in the store. The cross-subsystem capability model the registry once only anticipated has landed (`docs/capability.md`): the caller authorization and the approved-registrar set are now capabilities. The kernel layer's own verb rows stay hardcoded at `create()` by design; applications register theirs dynamically through the surface below, and the dispatch shape is the same either way.

### The application registration surface

**Verbs**: `console-ext` (status), `console-ext approve <domain>`, `console-ext unapprove <domain>` -- plus the registry LFUNs `extend(verb, path, method)` / `retract(verb)` the application calls.

An application domain ships a first-class operator verb without a kernel edit: the operator approves the domain once (`console-ext approve <Domain>` grants the `admin_console.extend` capability to the domain principal -- `docs/capability.md` The gating surfaces), and the domain's own code -- typically its initd or daemon at boot -- calls `ADMIN_CONSOLE_REGISTRY->extend(verb, path, method)` (the canonical `#define` is in `<kernel/user.h>`). The registered verb then dispatches exactly like a built-in extension verb: the handler sees the console object as `previous_program()`, so the same `KERNEL()` caller posture applies in the target.

`extend` enforces the registration contract and throws on any violation:

- the caller's program must live under `/usr/<domain>/` for a domain holding `admin_console.extend` (operator-approved);
- the registered `path` must live in that same domain -- no planting verbs into another domain's objects;
- the `method` must carry the `cmd_` prefix (the operator-verb naming contract; no registering arbitrary LFUNs);
- the verb is a single word, may not shadow a console built-in (those dispatch before the registry is consulted, so a shadow entry would sit silently unreachable), and may not take an already-registered verb.

Each entry records its registrar domain, and only that domain may `retract` it; the kernel layer's own rows record no registrar and are not retractable. `console-ext` with no arguments lists the registered extension entries and the approved domains. `console-ext unapprove <domain>` revokes the capability and sweeps the domain's registered verbs in the same act -- a domain stripped of the capability could no longer retract its own leftovers, since `retract` checks the same capability. A boot-time `extend` attempt from a not-yet-approved domain throws like any other refusal; `examples/console-ext-app` is the worked example, including the record-the-refusal-instead-of-failing-the-deploy boot pattern and the approve-then-register lifecycle.
...
**Swap is growing but object counts are flat.** The swap-sector percentage is climbing (`docs/operations.md` Monitoring signals alerts on it at 50/70%) while `rsrc objects` and per-owner `quota <owner>` counts hold steady: population is not the growth -- some object's dataspace is. This is exactly the shape `docs/configuration.md` (Sizing a workload) steers large record sets into: a mapping or `Array` holder lives in one dataspace and grows sectors without growing any count, while a `KVstore`'s growth would show in `rsrc objects` as node objects. The per-object attribution tool is `status <obj>`'s `Sectors` field, with one timing caveat captured live: sector counts reflect the swap device, so a resident object that has not paged out since it grew reads `Sectors: 0` -- or a stale prior count -- however large it is in memory (a 20,000-entry string array read `Sectors: 0` before a dump and `Sectors: 158` after it). So: `snapshot` first -- the dump writes every object and refreshes every count -- then `status <obj>` each of the application's known large holders, and where a suspect exposes a size accessor, confirm with `code "<path>"->size()` (or the collection's own count function) before acting; with no accessor, the before-and-after `Sectors` comparison across two snapshots is the fallback. What the platform does not provide, so design compensates: there is no per-owner byte or sector rollup and no verb that ranks objects by size -- attribution walks named suspects, which is why a large collection deserves a size accessor at design time, alongside the shape choice itself (`docs/kernel-libraries.md` Choosing a collection). The remediation ladder once the growth is attributed is `docs/operations.md` When the image approaches the state ceiling.
...
| `capability [delegable <cap> on\|off \| grant <cap> <principal> \| revoke <cap> <principal>]` | Capabilities | List the capability store (principal count and delegable flag per capability); `delegable` flips the per-capability delegation gate; `grant`/`revoke` add and remove store entries for domain and program principals (hosted by capabilityd) |
...
| `console-ext [approve\|unapprove <domain>]` | Console extensions | List registered extension verbs and approved domains; `approve` grants `admin_console.extend` to the domain principal, `unapprove` revokes it and sweeps the domain's registered verbs (hosted by the registry) |
...
| `identity <sub>` | Identity | Identity-record lifecycle: `mint <n>`, `show`, `rotate-codes`, `redeem`, `revoke`, `bind`, and `grant`/`ungrant <uuid> <capability>` (hosted by identityd; sub-verb detail in Dispatcher operator surface above) |
| `log [N]` | Logging | Tail the last N lines of the persistent log; the read is bounded so a large unrotated log is not loaded wholesale (hosted by logd) |
| `log-level [<level>]` | Logging | Show or set the persistent-log severity threshold (hosted by logd) |
...
| `session <sub>` | Sessions | Session-count and mint-ceiling status; `mint <principal> [ttl]`, `max-ttl [seconds]`, `validate`, `revoke`, `revoke-principal`, and `list <principal>` (hosted by sessiond; sub-verb detail in Dispatcher operator surface above) |
...
| `tls-cert [reload\|paths <cert> <key>]` | TLS | Certificate and registration status; `reload` revalidates the PEM files and completes a boot-deferred registration, `paths` re-points the configured paths and reloads (hosted by the HTTPS bootstrap) |
...
| `webauthn <sub>` | Identity | Relying-party status and configuration (`rpid`/`origin`); `register` and `authenticate` drive the ceremonies from base64url payloads (hosted by webauthnd) |
