...
That validation driver sits three commits behind the `733ea01e` pinned in the Standard build above: two of them are the `make DEFINES=` fix described under The `DEFINES` override below, with `src/Makefile` their entire delta, and the third closes a connection immediately on a write error (`src/comm.cpp`), which touches nothing the build or this recipe depends on. What changed is that on the pin the large-file flag arrives on its own, so restating it is redundant rather than required, and the recipes above are written to work either way. Build the pin unless you have a reason not to: it is what the container recipe and the regression workflow build.
