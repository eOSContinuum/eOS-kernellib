...
**No initializers in declarations.** Local variable declarations parse: `local_data_declaration` (`grammar/merry.y` line 150) accepts `string a, b;`. But no initializer production exists, so `object o = Spawn($this);` is a parse error. Declare, then assign: `object o; o = Spawn($this); return Set(o, "name", "x");`. Keep declarations before statements: the grammar does not pin declarations to block top (`dcltr_or_stmt`, line 173, feeds both `program` and `compound_stmt`), but the declaration text passes through verbatim to the emitted LPC, and the DGD compile stage requires declarations to precede statements in a block. `constant_declaration` (`constant IDENT = expression;`) is the one declaration form that takes an initializer, for compile-time constants. For one-shot values, inline composition remains the idiom: the reference application's `examples/merry-app/sys/test.c` phase 3 uses `"return object_name(Spawn($this));"` rather than a declare-assign-return sequence.
...
There is no `VAL_SAM` tag (the `$"..."` SAM-token surface is a game-content extension of the historical design, and eOS-kernellib does not carry it). The numbering above is contiguous.
