...
**Audience**: a developer evaluating or starting on the platform whose instincts were trained on cloud-service architectures; has skimmed [persistence.md](persistence.md) Why orthogonal persistence or is willing to take the persistence model on credit for the next ten minutes.
...
Honesty about the other direction — needs the service stack serves that this platform deliberately does not:
...
- **Polyglot services.** The runtime's guarantees hold for LPC (and Merry) code inside the image. External services integrate at the transport boundary like any other client.
