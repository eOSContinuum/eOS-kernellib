...
Operate both versions against the same inputs and measure. This stage is the least served by the platform today, and the honest framing is that you will build most of the instrumentation yourself.

### What the platform measures, and what it does not

[`../scripts/README.md`](../scripts/README.md) measure-baseline.py is the timing rig: it boots cold timed to console-ready, grows the image in steps, records the client-observed snapshot pause and snapshot size at each step, times a restore boot, and drives sequential requests against the bundled HTTP example. It characterizes *the platform's* envelope, and the numbers it produced are in [`configuration.md`](configuration.md#limits-and-capacity) Limits and capacity.

It is not a harness for your application. It drives the bundled example specifically, and pointing it at your own domain is not supported today. Plan to write your own load driver.

### Building your own comparison

- **Latency and throughput.** Your own driver against your own routes, on both systems, with the same inputs. The platform's published figures are a reference point for what the shape should look like, not a substitute for measuring your workload ([`configuration.md`](configuration.md#limits-and-capacity) Limits and capacity).
