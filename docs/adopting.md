...
- **Durability.** Persistence is deliberate statedump, not per-transaction commit: an unclean stop loses everything since the last completed dump, including writes already acknowledged to clients. The platform schedules no dumps itself, so running and sizing a snapshot cadence is the decision ([`operations.md`](operations.md#availability-and-data-loss-model) Availability and data-loss model); making one specific write durable at acknowledge time is a worked recipe with a trap in it ([`common-tasks.md`](common-tasks.md#make-one-write-durable-at-acknowledge-time) Make one write durable at acknowledge time).
...
- Did the durability model fit, or did snapshot-cadence sizing turn into a running argument with the business?
