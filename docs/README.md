...
- [capability.md](capability.md): the capability library (`capabilityd` store + inheritable check face) behind the capability-separation primitive: the mechanism, the seven gating surfaces, what "capability" means here (tier-mediation, not strict object-capability) and does not, the lifecycle, the identity-principal grant path, and the limitations.
