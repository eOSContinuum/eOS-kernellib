...
- [first-vault-entity.md](first-vault-entity.md), the other continuation of the third tutorial: give one of the key-value service's keys a second kind of durability -- a Vault node, a schema you declare, and a typed record written to its own XML file, still there after a cold boot with the snapshot deleted. Module-less, unlike `first-composition.md`, so the transcript regression covers it.
...
- [adopting.md](adopting.md): the twelve-week shape of a real trial, after `evaluating.md` says the platform fits: what to run in week 1, how to choose and translate one workflow, what to measure in a parallel run, and what should settle the migrate-or-step-back decision. Names where the platform leaves you to build your own instrumentation.
...
- [capability.md](capability.md): the capability library (`capabilityd` store + inheritable check face) behind the capability-separation primitive: the mechanism, the gating surfaces, what "capability" means here (tier-mediation, not strict object-capability) and does not, the lifecycle, the identity-principal grant path, and the limitations.
...
- [operations.md](operations.md): boot modes, state persistence, backup and restore, the availability and data-loss model, logging and diagnostics, host-driver extension loading. The deployment surface.
- [configuration.md](configuration.md): the `.dgd` configuration fields, resource limits, and capacity ceilings -- what the fields mean, what the caps are, what has been measured.
...
- `../examples/console-ext-app/`, the registered-console-verb worked example: a domain daemon that registers `ext-hello` through the kernel registry's capability-gated `extend()` surface, the operator approve/unapprove lifecycle, and the boot-before-approval refusal pattern. Read alongside [admin-console.md](admin-console.md) The application registration surface.
...
- **Evaluate whether the platform fits**: start at [evaluating.md](evaluating.md), the one-page brief that assembles the verdict (its closing section orders the depth reading by remaining budget). The depth links: [runtime-primitives.md](runtime-primitives.md) for what is proven today (`../scripts/run-example.sh` runs the proofs), [runtime-platform-roadmap.md](runtime-platform-roadmap.md) for the ships-today-versus-next boundary, [configuration.md](configuration.md) Limits and capacity for the envelope, [security-posture.md](security-posture.md) for the trust boundaries and operator responsibilities, [architecture.md](architecture.md) The driver dependency for the AGPL runtime's license boundary and continuity posture, [coming-from-contemporary-infrastructure.md](coming-from-contemporary-infrastructure.md) for what the platform replaces, and [debugging-applications.md](debugging-applications.md) The working environment, plainly for a team's day-to-day (editor reality, source-of-truth discipline, CI).
- **Run a real trial after deciding the platform fits**: [adopting.md](adopting.md), the twelve-week pathway from a first boot to a migrate-or-step-back decision, which sequences the docs below into a time-ordered arc and names where the platform leaves you to build your own instrumentation.
...
- **Model your data**: [where-code-belongs.md](where-code-belongs.md) Where state belongs for the representation fork (property table versus plain member), then [application-authoring.md](application-authoring.md) Modeling domain data for entity shapes, lookup, and secondary indexes, [kernel-libraries.md](kernel-libraries.md) Choosing a collection with [configuration.md](configuration.md) Sizing a workload for the ceilings, and [persistence.md](persistence.md) Getting data out for what each representation can export.
...
- **Write a Vault-persisted application**: [first-vault-entity.md](first-vault-entity.md) for the hands-on ramp, then [lpc-essentials.md](lpc-essentials.md), [persistence.md](persistence.md), [vault-applications.md](vault-applications.md), `../examples/vault-app/`.
...
- **Operate a running deployment**: [operations.md](operations.md), [admin-console.md](admin-console.md), [persistence.md](persistence.md), with [configuration.md](configuration.md) as the config-and-capacity reference beside them.
