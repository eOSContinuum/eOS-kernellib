...
- **A type-system dispatcher**. `dtd_daemon` registers type handlers (xml_daemon registers the core XML types, while the daemon itself handles the LPC primitives). Callers ask "what's the colour of `lpc_str`?" or "convert this ascii to typed value" and the daemon routes to the handler.
...
1. `compile_object("sys/dtd_daemon")`: DTD daemon is the type-handler registry. At `create()` time it inherits `/usr/XML/lib/entities` (a cross-domain inherit that works because System's initd grants `set_global_access("XML", TRUE)`).
...
**Write semantics.** `set_ascii_property` writes through `set_property`, so imported writes behave like any other write: observers fire, and the dispatch-path write gate applies. `import_state` is atomic: a throwing observer aborts the whole import. A `Core:Entries` import begins with `clear_all_properties` (the `Entries` callback): a reset of the marshalable property table that preserves `merry:` runtime wiring, the same namespace the export deliberately leaves out (see Enumeration filter above), so a wipe-then-import round-trips exactly what the export carried. Observer registrations survive a live re-import, stay coherent with the Merry daemon's resolved-observer cache, and are removed only through the daemon's gated unregister path.
