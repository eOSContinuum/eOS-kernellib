...
The delegated-grant seam is **deferred, not built**. Today an operator is the sole grantor of a platform capability to an identity; there is no path for one identity to grant a subset of its own capabilities to another. A future delegated-grant mode -- an identity attenuating and re-granting authority it holds -- would attach at the same elevation helper, adding a grantor check (does the delegating principal hold the capability, and is it delegable?) before `CAPABILITYD->grant`. It is not implemented because it needs a policy the platform has not chosen (which capabilities are delegable, whether delegation is transitive, how revocation cascades), and because the honest form of it is the held-reference mediation the model does not yet reach (see below). The seam is named here so a later wave attaches to a known point rather than reopening the grant path.
