...
- `src/lib/` holds the collection and value types: `String.c`, `StringBuffer.c`, `KVstore.c`, `KVnode.c`, the `Time.c` and `GMTime.c` pair, the `Iterator` family, and the `Continuation` family for asynchronous control flow. These are catalogued in `docs/kernel-libraries.md`.
