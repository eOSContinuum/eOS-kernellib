...
- The approved-registrar set -- the creator domains permitted to call `register_observer` from outside `/kernel` -- lives in the capability library under the `merry.registrar` capability, not in this daemon's own state. `_check_registrar` consults it via `CAPABILITYD->is_allowed`; the public `query_approved_registrars` forwards the read to `CAPABILITYD->query_principals`. Mutation is performed by the kernel mediator (the console registry) granting or revoking against the store under `KERNEL()` elevation; the Merry daemon holds no mutator of its own. Statedump-persistent (the store persists). See `docs/capability.md`.
