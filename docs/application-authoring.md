...
**Human and agent identity is the shared substrate.** One identity record per human or agent, minted by `identityd`, carrying typed credential rows and a principal string `identity:<uuid>` (`docs/identity.md`, `docs/system-daemons.md`). Every application consumes the same record rather than keeping its own user table. Passkeys (WebAuthn, TOFU registration) are the first credential type; `webauthnd` runs the ceremonies. An authenticated identity's principal is the seam into authorization -- an application gates its own actions on it (application-tier authorization), and an operator may grant it a platform capability through the store (`docs/identity.md` The three-layer authorization split).
