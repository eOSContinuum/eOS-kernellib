...
|---|---|---|
| A System daemon (`errord`, `logd`, `userd`) | Yes | `compile <path>` from the admin console. Master variables survive; `create()` does not re-run, so driver registrations held by reference persist. A daemon whose `create()` re-seeds state (capabilityd re-seeds static grants and drops runtime ones, `docs/capability.md`) carries that daemon's own caveat. |
