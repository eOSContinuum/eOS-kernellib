...
**Rehearse on a restored copy.** In a stateless stack, staging is obvious; here state lives in the image, so the release engineer's standard question -- how do I test this upgrade against production-shaped state before running it live? -- has a platform-specific answer the docs already contain in pieces. Restore the current backup set on a second host per the off-host restore drill (`docs/operations.md` Backing up and restoring state), run the identical cascade and verify steps there, then apply to production. The pre-release snapshot from step 1 is the only in-place safety otherwise, and Rolling back a release below shows why leaning on it alone is risky. The same path is the only safe way to test a host-binary upgrade: does the snapshot restore under the candidate driver build? Restore under the candidate binary on the second host; the `Bad or incompatible restore file header` refusal (`docs/operations.md`) makes an incompatible pair fail the rehearsal rather than production. A `.dgd` config change rehearses the same way.
...
- **Snapshot restore** (step 1's restore point) rewinds the whole image. Every write committed after the snapshot -- user state included -- is forfeited, potentially hours of it at an hourly snapshot cadence (`docs/operations.md` Availability and data-loss model). It is the correct recovery when the release failed midway or corrupted state broadly enough that the image's integrity is in doubt.
