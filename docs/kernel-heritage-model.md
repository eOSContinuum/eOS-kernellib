# The kernel heritage model

The conceptual overview of the kernel library, inherited from the upstream `dworkin/cloud-server` kernel documentation and kept for the contributor mapping the kernel tier to its origins. It takes a game-mudlib point of view throughout, the frame the library was written in; the platform's own tutorials and reference supersede its build guidance (`docs/getting-started.md`, `docs/first-hour.md`, `docs/architecture.md` The boot, in source order). It is background, not a procedure to follow.
