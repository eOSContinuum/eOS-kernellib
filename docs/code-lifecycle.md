...
The two-argument form, `compile_object(path, source)`, compiles from an in-memory string rather than from disk. It is useful for platforms that synthesize LPC at runtime.
...
A clone's `create()` runs in its own dataspace at clone time. The master's `create()` runs once when the first call against the master fires after compile. Whether that is at compile time depends on the application's pattern: an initd that does `compile_object("/usr/MyApp/obj/foo")` and then makes a method call against the master triggers `create()` at that point. The kernel auto handles the master/clone distinction via its registration logic, so most application authors write a single `create()` body that runs on both master and clones.
...
The working demonstration is `examples/upgrade-cascade/`: a parent library upgraded live through the daemon, its inheritor recompiled, the inheritor's existing clones reporting the new library's version and behavior with their per-clone state intact and `patch()` run exactly once each. Both entry points are exercised: the boot driver reaches the daemon through the auto library's `upgrade()` wrapper, and `scripts/verbsets/operator-upgrade.verbset` drives the operator `upgrade -p` verb against the same deploy as a registered non-admin operator (per-owner access refusal included), asserting the cascade landed on the example's Index-named clones.
