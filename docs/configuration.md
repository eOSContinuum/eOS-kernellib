...
| `dump_interval` | Seconds the driver spreads the post-restore rebuild over -- NOT a snapshot schedule; the platform takes no automatic snapshots (`docs/persistence.md` The rebuild interval). 3600 is a reasonable default |
...
The stock driver build's index widths (matching the driver's own header comment: "default: 64K objects, 64K swap sectors, 255 users, max string length 64K") set the ceilings above. A driver rebuilt with wider `uindex`/`eindex` types raises them, at the cost of a larger per-object memory footprint; `docs/building.md` Wider index types carries the working recipes and what each one is validated to do. eOS-kernellib runs against a stock build, so the table above is the practical ceiling for this project's own testing.

Three different things are called a ceiling here, and a deployment reaching one needs to know which it hit, because the remedies do not resemble each other:

- **In use** -- what the running workload actually consumes, read off the health vector (`docs/operations.md` Monitoring signals). This is the number to watch; the other two only matter once it approaches them.
- **Configured** -- what the `.dgd` file allocates. `objects = 10000` in `example.dgd` sits well under its compiled ceiling, so an object-table alarm on the demo config is answered by editing a number and rebooting from snapshot, with no rebuild.
- **Compile-time** -- what the binary permits. `users = 255` is already at `EINDEX_MAX`, so nothing in the config moves it; only a rebuild does, and `EINDEX_TYPE` is not what the wider-index recipes widen.

The distinction bites hardest on connections. Measured against this project's own composite example driven through an authenticated write path, the connection ceiling binds roughly two orders of magnitude before the object one -- at 250 concurrent connections the object table sits at about 1.3% of its compiled ceiling. So a deployment that widens `uindex` to buy headroom has very likely widened the ceiling that was not binding (`docs/operations.md` Connection-slot economics).
...
dump_interval   = 3600;     /* post-restore rebuild spread; NOT a dump timer */
```
...
**Unmeasured today.** Sustained behavior held over time near the `objects` / `call_outs` / `array_size` ceilings is not measured against this codebase -- the growth and concurrency runs above are single-pass, not extended-duration. The memory cost of a driver rebuilt with wider index types now has a floor rather than a figure: about 80 KB of additional resident memory at rest, and roughly 5.7% more snapshot for identical state, both measured on a small object graph (`docs/building.md` Wider index types). Since the widened types are per-object and per-sector, the cost at a realistic graph size is a different and still-unmeasured number, as is any wide-build behavior on the crypto-gated surfaces. No run has held the platform under continuous load for days or weeks: dynamic-arena growth past the chunk-ahead behavior above, swap-file fragmentation, call_out table churn, and log or audit growth against the dump cycle have no published multi-day trajectory, so an operator tuning alert thresholds has no baseline to check a slowly climbing signal at day ten against. The sector-size dependence of the pause at the ~512 MB scale (0.037 s at `sector_size` 8192 vs 0.541 s at `sector_size` 32768) is an open observation from single runs, not yet explained. Treat the tables above as compiled-in ceilings and documented defaults, not throughput guarantees.
