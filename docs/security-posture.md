...
- **Human and agent authentication (the identity substrate).** Passkey (WebAuthn) credentials bound to a shared identity record, with strict ceremony verification (challenge, origin, rpIdHash, signature counter replay), atomic rotation, and a recovery order that never bare-re-binds a fresh credential to an existing identity nor recovers by minting a new one (`docs/identity.md`). Session tokens are stored hashed, never in plaintext -- absent from the statedump as a tested property (`docs/system-daemons.md` sessiond). This is a distinct circuit from operator authentication below: an application-user identity is never a kernel user or owner and never reaches the console.
