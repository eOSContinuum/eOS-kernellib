...
- **Per-call access checks.** A `code` invocation hits a per-call access check at every privileged kfun boundary (file, compile, connection, and resource operations), so a non-admin operator cannot exceed its owner's grants through the console (`docs/admin-console.md` Security posture).
