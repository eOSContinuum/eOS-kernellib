/*
 * Parser-runner library: a buffer over the DGD parse_string() kfun
 * that compiles a yacc-shape grammar to a scratch LPC object and
 * dispatches semantic actions to its f<N>() handlers.
 *
...
if (sscanf(middle, "%*s__STRINGS__")) {
   ...
   if (find_object(scratch_file)) {
      ...
      sysLog("find_object() now returns... " +
	     dumpValue(find_object(scratch_file)));
   }
