...
/*
 * remove the current node (do nothing)
 */
static void delNode() { }
		    ...
		    object next, int change)
{
	...
	delNode();
	return ({ MERGE_RIGHT, nil, nil, change });
    }
}
	...
	if (type <= LEAF) {
			   ...
			   keyValue(keys[index - 1], values[index - 1]) == nil)
		{
			...
			if (keyValue(keys[maxSize], values[maxSize]) == nil) {
			    ...
			    key = keys[index];
			    value = newNode(accessKey, maxSize, keys[index ..],
					    values[index ..]);
			    keys = keys[.. index - 1];
			    values = values[.. index - 1];

			    if (type == LEAF) {
				return ({ SPLIT, key, value, change });
			    }
			    ...
			    type = ROOT;
			    values = ({
				newNode(accessKey, maxSize, keys, values),
				value
			    });
			    keys = ({ key });
			}
		...
		if (type != FLAT && sizeof(keys) < minSize) {
		    return grow(index, prev, nil, nil, next, change);
		}
	    } else if (exists > 0) {
		error("Key not present");
	    }
		...
		if (sizeof(keys) > maxSize) {
		    index = minSize;
		    key = keys[index];
		    value = newNode(accessKey, maxSize, keys[index + 1 ..],
				    values[index + 1 ..]);
		    keys = keys[.. index - 1];
		    values = values[.. index];
