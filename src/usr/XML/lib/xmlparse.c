/*
 * XML parser. Converts ASCII XML to the internal XMD data format.
 *
...
static mixed parse_xml(mixed str, varargs string file, int peekflag, int looseflag)
{
...
private void p_oneof()
{
    ...
    LexErr("`{ ... | ... }` SAM sugar-tag syntax is not supported by the lifted XML transport layer");
}

private int is_merry_expression(string str)
{
    int j;
...
private void p_ref()
{
	...
	Expect(')'); Scan();
	Nugget(xmdRef(str, attr));
	return;
    case 'a'..'z': case 'A'..'Z':
    case '\"': case '\'':
	str = ScanName();
	Nugget(xmdRef(str, nil));
	return;
    case '[':
	...
	Nugget(xmdRef("Merry.Eval", ({
	    "node",
	       new_object("/usr/Merry/data/merry", str),
	       "line", _line,
	       })));
	Scan();
	return;
...
private void p_tag()
{
	...
	if (char == '/') {
	    ...
	    Expect('>');
	    Scan();
	    Nugget(xmdText(tag, attr, nil));
	    Debug("Nogget: " + dumpValue(nugget));
	    return;
	}
	if (char == '>') {
	    Scan();
	    ...
	    XDebug("after push, stack is " + dumpValue(stack));
	    return;
	}
	LexErr("expecting / or >");
    case '/':
	...
	Nugget(xmdText(tag, attr, content));
	break;
    default:
	LexErr("unexpected token");
	break;
...
private mixed convert(mixed content, varargs string ltype, int strip)
{
    /*
     ...
     *	conversions defined in ~Schema/dtd -- we check for OBJ() and NREF().
     *
    ...
    while (typeof(queryColourValue(content)) == T_ARRAY) {
	/* loop while there are still useful optimizations */
	if (sizeof(queryColourValue(content)) == 0) {
	    content = "";
	} else if (sizeof(queryColourValue(content)) == 1) {
	    /* collapse single-element arrays */
	    content = queryColourValue(content)[0];
	} else {
    ...
    } else if (!ltype && !queryColour(content)) {
