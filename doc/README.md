...
This directory contains the reference documentation for eOS-kernellib — the kernel layer for orthogonally-persistent servers built on the [DGD] driver. The root `README.md` introduces the project and what the substrate provides; the docs here cover the substrate's model, operator surface, and application-authoring patterns in depth.

**Audience**: a reader entering the documentation directly — either to set up the substrate, understand its architecture, write an application on top, or operate a running deployment.
...
- [getting-started.md](getting-started.md) — first-time install of DGD and eOS-kernellib; run the bundled example configuration. Start here if the substrate is not yet booted on your machine.
...
- [architecture.md](architecture.md) — capability tiers, daemons, boot sequence, auto-inheritance, System global-access, host-driver extensions. The structural reference for the substrate.
- [substrate-primitives.md](substrate-primitives.md) — the eight runtime primitives (atomicity, capability separation, persistent state, hot reload, sandboxed code load, asynchronous events, multi-agent coherence, state introspection), each with foundation, demonstration status, supporting extensions, and open work.
...
- [http-applications.md](http-applications.md) — HTTP/1-specific patterns: mount-point convention, application layout, the server object's role, body-bearing methods, the four substrate contracts.
...
- **Run the substrate and see it work** — [getting-started.md](getting-started.md), then `../examples/http-app/README.md`.
- **Understand the substrate's architectural commitments** — [architecture.md](architecture.md), then [substrate-primitives.md](substrate-primitives.md).
...
- **Reason about hot reload and code evolution** — [code-lifecycle.md](code-lifecycle.md), then the hot-reload sections of [substrate-primitives.md](substrate-primitives.md).
