...
For deeper platform dispatch (how the kernel auto handles master vs clone, how owner identity is set, how the clone registry is maintained), see `doc/architecture.md` "Auto-inheritance pattern."
...
For the platform-level guarantee and its open empirical questions under host-driver extensions, see `doc/runtime-primitives.md` §1.
...
- **`doc/architecture.md`** — the platform's tier model, daemons, the auto-inheritance chain, the host-driver extension surface.
- **`doc/application-authoring.md`** — what writing a tier-E application on top of this platform looks like: domain layout, initd, owner / access, the object-manager lifecycle, `call_touch` upgrade.
...
- **`doc/runtime-primitives.md`** — per-primitive foundation-and-status statement (atomicity, persistent state, hot reload, capability separation, etc.).
