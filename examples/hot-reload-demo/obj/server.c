/*
 * HTTP/1 server for the hot-reload demonstration.
 *
 ...
 *
 * Inheritance pattern and binary-manager glue are replicated strict-
 * substitution from examples/http-app/obj/server.c (the verified reference).
 * The only differences from http-app are the routing surface in dispatch()
 * and the inclusion of the greeting master as the route target.
 *
 * Routes:
 *   GET  /greet     -- return /usr/WWW/greeting->greet() as plain text.
 *   POST /compile   -- compile_object("/usr/WWW/greeting", body) using the
 *                      request body as new LPC source. The next /greet
 *                      dispatches to the new program; existing in-flight
 *                      calls finish on the old program (host-runtime
 *                      guarantee, see docs/runtime-primitives.md section 4).
...
{
    string method, path, replyBody, source;
    object obj;
    mixed err;
	...
	err = catch(replyBody = GREETING->greet());
	if (err) {
	    replyBody = "greet() failed: " + err + "\n";
	    emit(makeResponse(500, "Internal Server Error", replyBody),
		 replyBody);
	    return;
	}
	...
	source = drainBody(body);
	if (!source || strlen(source) == 0) {
	    replyBody = "POST /compile requires a request body (LPC source)\n";
	    emit(makeResponse(400, "Bad Request", replyBody), replyBody);
	    return;
	}
	err = catch(obj = compile_object(GREETING, source));
	if (err) {
	    replyBody = "Compile error: " + err + "\n";
	    emit(makeResponse(500, "Internal Server Error", replyBody),
		 replyBody);
	    return;
	}
	if (!obj) {
	    replyBody = "compile_object returned nil\n";
	    emit(makeResponse(500, "Internal Server Error", replyBody),
		 replyBody);
	    return;
	}
	replyBody = "Compiled " + object_name(obj) + "\n";
	emit(makeResponse(200, "OK", replyBody), replyBody);
	return;
    }
