...
The substrate's HTTP/1 bootstrap (`src/usr/System/sys/http_server.c`) clones the application at the kernel-defined path `/usr/WWW/obj/server`. To deploy this application, copy the directory into the kernel layer's `src/usr/`:
...
- `obj/server.c` -- per-connection HTTP/1 server (clonable). The substrate clones one per incoming connection.
