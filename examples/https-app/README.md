...
3. **A certificate.** PEM certificate and key at the configured paths -- `/usr/System/data/tls/cert.pem` and `key.pem` by default (paths as the platform sees them, i.e. under the configured `directory`; re-point with the `tls-cert paths` operator verb). Acquisition and renewal belong to the host -- an ACME client such as certbot writing those paths. A certificate may arrive after boot: the bootstrap stands down without one, and `tls-cert reload` on the admin console validates the files and completes the registration with no restart; renewed files need no verb at all, since credentials are read per connection. For a local run, a self-signed P-256 certificate works:
