...
`DGD_BIN=/path/to/dgd/bin/dgd scripts/run-example.sh <example>` is the one-command harness -- clean-slate deploy, boot cycle, sentinel-count assertion -- for twelve of the fifteen examples (see the profile table at the top of the script for which). `https-app` verifies via `scripts/https-smoke.sh` and `console-ext-app` via its verbset under `scripts/drive-verbs-smoke.sh`, while `kv-tutorial` is not a harness example at all. `http-app`'s profile drives live HTTP requests and writes the result log itself rather than reading a test driver's; `atomic-demo` and `hot-reload-demo` verify both ways. `../docs/README.md`'s Working examples section is the authoritative description and reading-path map; the rows below are one-liners, and each example's own README plus the harness profile carry the mutable sentinel counts (the verify command surfaces them).
