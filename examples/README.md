...
`DGD_BIN=/path/to/dgd/bin/dgd scripts/run-example.sh <example>` is the one-command harness -- clean-slate deploy, boot cycle, sentinel-count assertion -- for eight of the ten examples (see the profile table at the top of the script for which). The other two, `atomic-demo` and `http-app`, verify against a running server via HTTP probes; `hot-reload-demo` verifies both ways. `../docs/README.md`'s Working examples section is the authoritative map from each example to its companion doc and reading path.
